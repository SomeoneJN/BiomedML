...
from Ver3_noise_and_plot_functions import (
    add_noise_simple, create_noisy_shape, get_smooth_curve, plot_perfect_circle,
    compute_means_pair, plot_comparison, plot_means
)

# ---------------------
# Option: Number of Sets to Test (default 16) and Custom Noise Level (default 0.1)
# ---------------------
num_sets = 4  # Change this value to test a different number of sets.
custom_noise_level = 0.5  # Default noise level for shape data (can be modified).
...
# ---------------------
# For Bottom Cylinder, we now want inner_z as x-axis and inner_y as y-axis.
bottom_x_cols = [f'inner_z{i}' for i in range(1, 10)]  # inner_z as x-axis
bottom_y_cols = [f'inner_y{i}' for i in range(1, 10)]  # inner_y as y-axis
...
# Now, since the axes are swapped, inner_z columns are used for x-axis and inner_y for y-axis.
subset_inner_z = df[bottom_x_cols].iloc[:1000]  # bottom_x_cols = inner_z columns
noise_inner_z = round(subset_inner_z.std(axis=0).mean(), 2)
print("Noise level for inner_z (x-axis):", noise_inner_z)
subset_inner_y = df[bottom_y_cols].iloc[:1000]  # bottom_y_cols = inner_y columns
...
# ---------------------
# Process Inner Shape:
# For normal inner shape, use noise_level=0 (ideal closed spline).
# For noisy inner shape, use noise_level=custom_noise_level.
...
# ---------------------
# Process Outer Shape:
# For normal outer shape, use noise_level=0.
# For noisy outer shape, use noise_level=custom_noise_level.
for i in range(len(subset_df)):
    row = subset_df.iloc[i]
    # Normal outer shape.
...
for idx, (_, row_norm) in enumerate(bottom_normal.iterrows()):
    ...
    # Note: For bottom cylinder, x-axis uses inner_z columns and y-axis uses inner_y columns.
    x_norm = row_norm[bottom_x_cols].to_numpy(dtype=float)  # inner_z
    y_norm = row_norm[bottom_y_cols].to_numpy(dtype=float)  # inner_y
...
# ---------------------
# Figure 3: Grid Plot for Inner Shape (Smooth Normal vs Noisy with Sample Markers)
# ---------------------
...
# Use different colors for normal and noisy splines.
# ---------------------
...
# ---------------------
# Figure 5: Grid Plot for Outer Shape (Smooth Normal vs Noisy with Sample Markers)
# ---------------------
