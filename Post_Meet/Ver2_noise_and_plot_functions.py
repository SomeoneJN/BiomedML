# -*- coding: utf-8 -*-
"""
Created on Wed Apr  9 13:50:01 2025
...
def get_smooth_curve(x_points, y_points, num_points=200):
    ...
    (assuming the points define a closed curve) and return a dense smooth curve.
...
def plot_perfect_circle(ax, x_points, y_points, label='Perfect Circle', circle_color='purple'):
    """
    Compute a perfect circle from sample points by computing the center (mean)
    and average radius. Then, plot that circle on the axis with markers at the sample points
    (annotated with their (x, y) coordinates).
    
    Parameters:
        ax: A Matplotlib axis.
        x_points, y_points: 1D arrays representing the 10 sample points.
        label: Legend label.
        circle_color: Color for the circle and its markers.
    """
    ...
    for (x_val, y_val) in zip(x_points, y_points):
        ax.annotate(f"({x_val:.2f}, {y_val:.2f})", (x_val, y_val),
                    textcoords="offset points", xytext=(0,5), ha='center', fontsize=6)

def compute_means_pair(data, cols_x, cols_y, groups=5):
    """
    Split the DataFrame into 'groups' nearly equal parts and compute the column‐wise mean
...
def plot_means(ax, norm_mean_x, norm_mean_y, noisy_mean_x, noisy_mean_y, noisy_examples, cols_x, cols_y, title):
    ...
    The smooth curves are generated by fitting a periodic spline to the sample points.
    ...
    for idx, row in noisy_examples.iterrows():
        ...
        # Plot the 10 sample points.
        ax.scatter(x_sample, y_sample, color='red', marker='s', s=40)
