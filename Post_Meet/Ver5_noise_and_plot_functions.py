# -*- coding: utf-8 -*-
"""
Created on Thu Apr 10 00:27:24 2025
...
def create_noisy_shape(row, shape_x_cols, shape_y_cols, noise_level=0.1):
    ...
    Returns:
        x_noisy: 1D NumPy array of 10 sample x values.
        y_noisy: 1D NumPy array of 10 sample y values.
    """
...
def get_circular_spline_from_points(x_points, y_points, num_points=200):
    ...
    Compute a circular spline from 10 sample points as follows:
      ...
      4) Fit a periodic cubic spline (using CubicSpline with bc_type='periodic')
         to interpolate radii as a function of angle.
...
def align_noisy_to_normal(noisy_x, noisy_y, normal_x, normal_y):
    """
    (Optional) Align noisy sample points to the normal ones.
...
def plot_perfect_circle(ax, x_points, y_points, label='Perfect Circle', circle_color='purple'):
    """
    Compute a perfect circle from sample points (using mean center and average radius),
    then plot that circle on the axis with markers at the sample points (without annotations).
    If the label is set to '_nolegend_', it will not add a legend entry.
    """
...
def compute_means_pair(data, cols_x, cols_y, groups=5):
    """
    Split the DataFrame into 'groups' nearly equal parts and compute column-wise means
...
def plot_comparison(ax, normal_df, noisy_df, cols_x, cols_y, title):
    ...
    for _, row in normal_df.iterrows():
        ...
        ax.plot(x, y, marker='o', linestyle='-', color='blue', alpha=0.3, label='Normal' if first_normal else "")
        first_normal = False
    for _, row in noisy_df.iterrows():
        x = row[cols_x].to_numpy(dtype=float)
        y = row[cols_y].to_numpy(dtype=float)
        ax.plot(x, y, marker='s', linestyle='--', color='red', alpha=0.3, label='Noisy' if first_noisy else "")
        first_noisy = False
    ax.set_title(title)
...
def plot_means(ax, norm_mean_x, norm_mean_y, noisy_mean_x, noisy_mean_y, noisy_examples, cols_x, cols_y, title):
    ...
    for i in range(norm_mean_x.shape[0]):
        ...
        ax.plot(x_dense, y_dense, color='navy', linewidth=3, label='Normal Mean' if i==0 else "")
    ...
    for i in range(noisy_mean_x.shape[0]):
        ...
        ax.plot(x_dense, y_dense, color='darkred', linestyle='--', linewidth=3, label='Noisy Mean' if i==0 else "")
    for idx, row in noisy_examples.iterrows():
        ...
        ax.plot(x_dense, y_dense, marker='x', linestyle=':', color='orange', alpha=0.7, label='Noisy Example' if idx==0 else "")
    ax.set_title(title)
    ax.set_xlabel(cols_x[0].split('_')[0])
    ax.set_ylabel(cols_y[0].split('_')[0])
    ax.legend(loc='upper right')
