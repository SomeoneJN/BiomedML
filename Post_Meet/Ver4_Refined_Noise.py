# -*- coding: utf-8 -*-
"""
Created on Wed Apr  9 23:06:51 2025
...
from Ver4_noise_and_plot_functions import (
...
# ---------------------
num_sets = 64  # Modify this value to test a different number of sets.
custom_noise_level = 0.5  # Default noise level for noisy shape data.
...
# Load Data
# ---------------------
data_file = "2025_3_3_intermediate.csv"  # Update file path if needed.
...
# ---------------------
# Process Inner Shape:
# Normal inner shape: use noise_level=0 (ideal closed spline).
...
# ---------------------
# Figure 3: Grid Plot for Inner Shape (Normal vs Noisy)
# Use perfect circular spline for both normal and noisy; plot the 10 normal sample points.
# ---------------------
...
for idx, (_, row_norm) in enumerate(inner_shape_normal.iterrows()):
    ...
    # For normal inner shape, compute perfect circular spline:
    ...
    # Also plot the sample points without annotations:
    ax.scatter(x_norm_sample, y_norm_sample, color='blue', marker='o', s=40)
    # For noisy inner shape, compute perfect circular spline:
    ...
    ax.set_title(f"Set {idx+1}")
    ax.legend(loc='upper right')
fig3.suptitle("Figure 3: Inner Shape Sets (Perfect Circular Splines, Grid)", fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.show()
...
# ---------------------
# Figure 5: Grid Plot for Outer Shape (Normal vs Noisy)
# Use perfect circular spline for normal and noisy; plot the 10 normal sample points.
# ---------------------
...
for idx, (_, row_norm) in enumerate(outer_shape_normal.iterrows()):
    ...
    ax.set_title(f"Set {idx+1}")
    ax.legend(loc='upper right')
fig5.suptitle("Figure 5: Outer Shape Sets (Perfect Circular Splines, Grid)", fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.show()
