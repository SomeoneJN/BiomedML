...
def getRadiiFromEdges(edge_elements_dictionary, cylinder_height, logFile, workingInputFile, obj):
    ...
    # print("topedges_dictionary:", topedges_dictionary)
    # CylinderFunctions.plot_3d_points(topedges_dictionary)
    max_value = -float('inf')
    for key, value in topedges_dictionary.items():
        if value[1] > max_value:
    ...
    print("workingInputFile:", workingInputFile)
    print("obj:", obj)
    ...
    # print("final_step_coords:", final_step_coords)
    # print("topedges_dictionary:", topedges_dictionary)
...
#### Changed by Gordon because the inner and outer points were not always correct
#### because the deformed shape messed them up; now using the intial file to get
#### the inner and outer points and then replacing them with the final coordinates
def getRadiiFromEdges_v2(edge_elements_dictionary, cylinder_height, logFile, workingInputFile, obj):
    # Get dictionary containing nodes of top inner & outer radii
    topedges_dictionary = {}
    for edge, value in edge_elements_dictionary.items():
        if math.isclose(value[2], cylinder_height, abs_tol=5e-6):
    ...
    print("workingInputFile:", workingInputFile)
    print("obj:", obj)
    # print(topedges_dictionary)
    startpoint = max_key
    outer_radius_dict, inner_radius_dict = IO.find_closest_points(topedges_dictionary, startpoint, 0.4)
    # print(outer_radius_dict)
    # Reassign dictionary coordinates to reflect those from the last step of log file
    final_step_coords = gic.extract_coordinates_from_final_step(logFile, workingInputFile, obj)
    # print("final_step_coords:", final_step_coords)
    # print("topedges_dictionary:", topedges_dictionary)
    for array in final_step_coords:
        if int(array[0]) in outer_radius_dict:
            # print(array[0])
            outer_radius_dict[int(array[0])] = array[1]
        elif int(array[0]) in inner_radius_dict:
            inner_radius_dict[int(array[0])] = array[1]
