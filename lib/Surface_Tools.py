...
#        print('Z CUTOFF:', z_cutoff)
        if val < z_cutoff:
            ...
            Wave_func = lambda x_final : integrate.quad(lambda x: sqrt(1+(-Amplitude*pi/wave_width*sin(pi*(x - center)/wave_width))**2),0,x_final)[0]-x_val
            x_final_initial_guess = x_val*(1-max_scaling)
            new_x_coord = round(optimize.fsolve(Wave_func, x_final_initial_guess)[0],3)
#            print('********************')
#            print(new_x_coord)
            delta_y = Amplitude*cos((new_x_coord - center)*pi/wave_width)
#            print('Amplitude:', Amplitude)
#            print('delta y curved down:', delta_y)
...
####################################################################
####################################################################
##           Modified p2 for cosine curve
#            p2 = Point(new_x_coord, end_y + delta_y - Amplitude/2, minZ_generic)
####################################################################
####################################################################
####################################################################

#           I still need to rotate this point about the z axis to get it aligned
#            Do I do that before or after I rotate about the z? I'm not sure it matters, but I like rotating about the z first
...
#            p2 = Point(x_val, end_y, minZ_generic)
            ...
            # ===============================================================
            #AVW_Angle = math.degrees(math.atan2(-1, slope))
...
#                print('p1:', p1)
#    
#    #            print('p2 1st rot:', p2_first_rotation)
#                print('cat angle:', cat_angle)
#    #            print('delta x:', p2_first_rotation.x - p1.x)
#    #            print('delta y:', p2_first_rotation.y - p1.y)
#                print('p2 original after curve (before rotation):', p2)
#                print('p2 before cat rotate (post droop rot): ', p2_first_rotation)
#                print('point in question:', x_val, y_val, val)
            p2_rotated = rotate_point_about_z(p2_first_rotation.x, p2_first_rotation.y, p2_first_rotation.z, cat_angle, p1)
#            print('after cat rotate: ', p2_rotated)
#            p2_first_rot_vector = [p2_first_rotation.x, p2_first_rotation.y, p2_first_rotation.z]
#            r = R.from_euler('z', cat_angle, degrees = True)
#            second_rotation = r.apply(p2_first_rot_vector)
#            p2_rotated = Point(second_rotation[0],second_rotation[1],second_rotation[2])
...
#            p2_rotated = rotate_point(p2.x, p2.y, p2.z, -AVW_Angle, p1)
            ...
            sl = pythag(p2_rotated.y - p1.y, p2_rotated.z - p1.z)
            if sl <= length_of_droop:

                try:
                    ccc = CatenaryCurve(p1,p2_rotated,length_of_droop)
                except Exception:
                    print("index = " + str(index))
                    p2_rotated = rotate_point(p2.x, p2.y, p2.z, AVW_Angle, p1)
#                    p2_rotated = rotate_point(p2.x, p2.y, p2.z, -AVW_Angle, p1)
                    try:
                        ccc = CatenaryCurve(p1,p2,length_of_droop)
                    except Exception:
                        print("----------")
                        print(index)
                        print(p1)
                        print(p2_rotated)
                        print(length_of_droop)
                        print("x+x+x+x+x+x+x+x+x+x+x+x- ERROR  #1 -x+x+x+x+x+x+x+x+x+x+x+x+x+x+x+")
                        ...
                        print("Could Not Calculate a, i, and j from ")
                        print("Mode of Failure: a cosh() function is used and it explodes when large numbers are placed passed to it.")
                        print(p1, "to", p2)
                        print("with length ", length_of_droop)
                        print("x+x+x+x+x+x+x+x+x+x+x+x- END ERROR -x+x+x+x+x+x+x+x+x+x+x+x+x+x+x+")
                    generated_surface.zAxis[index] = (p1.z + p2.z)/2.0
                    
                # put step 10.1 back here
...
#                print('drooped point 0 (tilt back):', p2_rotated.x, yz_tuple[0], yz_tuple[1])
                point_unfirst_rotated = rotate_point_about_z(p2_rotated.x, yz_tuple[0], yz_tuple[1], -cat_angle, p1)
...
#z_cutoff = optimize_cutoff_value(z_cutoff, slope, generated_interpolator_zx_y, maxz_at_x0_plane, rotated_generic_minz_on_hiatus_x, minz_at_x0_plane, p2, max(generated_surface.zAxis))
#    threshold = .1
#    How much the surface changes in the z direction each step
#    surface_delta_z = slope * -INCREMENT
...
#
        ...
        print('angle difference', AVW_Angle - curve_angle)
# Trying a change 6/10/20
#        if AVW_Angle - curve_angle < -5:
        if AVW_Angle - curve_angle < -45:
            start_z += INCREMENT
# Trying a change 6/10/20
#        elif AVW_Angle - curve_angle > -1:
        elif AVW_Angle - curve_angle > -15:
            start_z -= INCREMENT
        else:
            print('STOPPING')
            return start_z
