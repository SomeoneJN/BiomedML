...
#            direct length between points
            ...
            # Given the ends of the arc (p1 and p2) and the length of 
...
#            print('[][][][][][][][][][] sl=', sl, 'droop length=', length_of_droop, '[][][][][][][][][][][]')
            if sl <= length_of_droop:
                ...
                # bisection line runs through the bisection point and is
                # perpendicular to the cord line. It contains the center
                # of the circle
                # (y,z)
                ...
                cord_slope = cord_vector[0]/cord_vector[1]

                # The bisector line must be perpendicular to the slope of the
                # cord line
                if cord_slope == 0:
                ...
                # finding the y intercept for the bisection line that the
                # center of the circle must be on
                # Before 1/11/21 changes
                # y_intercept = bisection_point[1]-bisector_slope * bisection_point[0]
                y_intercept = bisection_point[0]-bisector_slope * bisection_point[1]
                
                data = (point_1, point_2, arc_length, bisector_slope, y_intercept)

                zGuess = np.array([point_1[1] - 1])
                ...
                r = np.linalg.norm([point_1[1], point_1[0]] - center)
#
                
    #             circle_center(z2, *data):
    # # print('z:',z2)
    ...
    #             data = (point_1, point_2, arc_length, bisector_slope, y_intercept)
                # print(data)
                # print('testing if arc is correct:', circle_center([x_c], args = data))
    
                # print('cord slope:', cord_slope)
                # print('bisector slope:', bisector_slope)
                # print('bisection_point:', bisection_point)
...
#    # print('m and b:', m, b)
    ...
    # This is just the intial guess..it is the horizontal (z coordinate) for
    # the center of the circle
    x_c = z2[0]
    y_c = m * x_c + b
    
    # This is setting up the center as the [z,y] I believe
    # center = np.array([x_c, y_c])
...
#    print('center:', center)
    ...
    # p1 and p2 are the [y,z] couples
    #  finding the vectors from the center to p1 and p2
    vector_1 = p1-center
    vector_2 = p2-center
    # radius is the length of either of those vectors
    r = np.linalg.norm(vector_1)
    
    # getting the unit vectors in the directions of the center to the points
...
#    print('side of line:', side_of_line)
    ...
    # the angle between the two lines is the arc cosine of the dot product
...
#        print(arc_length)
        ...
        data = (point_1, point_2, arc_length, bisector_slope, y_intercept)
        zGuess = np.array([bisection_point[1]])
...
#        print('center:', x_c, y_c)
        ...
        # Slope of line from circle center to point leaving the avw
        slope_of_radius = (point_1[0] - y_c)/(point_1[1] - x_c)
