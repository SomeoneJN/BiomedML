...
"""
import lib.Surface_Tools as ss
import numpy as np
...
#        Get the data for the the part (typically the AVW)
        generated_surface = io.get_dataset_from_file(file_name, part_name).generatedCopy()
#       See how many nodes there are int he part
        unmodifiedLength = len(generated_surface.xAxis)
...
'''
Function: takeMeasurements_return
'''
def takeMeasurements_return(output, AVW, Fibers, GenericINPFile, OutputINPfile):
    ...
    average_strains = []
    for i, fiber in enumerate(Fibers):
        average_strains.append(average(newFiberLengths[i])/average(oldFiberLengths[i]))
