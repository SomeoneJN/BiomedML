...
def create_coordinate_dict(row1, row2, start_key=0):
    """
    Creates a dictionary of {key: [y, z, 4.0]} from two Series rows.
    """
    if not isinstance(row1, pd.Series) or not isinstance(row2, pd.Series):
        raise TypeError("Both inputs must be pandas Series.")

    y_values = row1.tolist()
    z_values = row2.tolist()
...
# def add_noise_to_coordinate_dict(coord_dict, noise_level1=0.1, noise_level2=0.1):
#     """
#     Adds noise to the y and z values in a coordinate dictionary.
#     """
#     noisy_coord_dict = {}
#     for key, values in coord_dict.items():
#         noisy_y = values[0] + np.random.normal(0, noise_level1)
#         noisy_z = values[1] + np.random.normal(0, noise_level2)
#         noisy_coord_dict[key] = [noisy_y, noisy_z, values[2]]

#     return noisy_coord_dict
...
def add_noise_to_coordinate_dict(coord_dict, noise_level1=0.1, noise_level2=0.1):
    ...
    """
    noisy_coord_dict = {}
    if isinstance(coord_dict, dict): # Check if it's a dictionary
        for key, values in coord_dict.items():
            noisy_y = values[0] + np.random.normal(0, noise_level1)
            noisy_z = values[1] + np.random.normal(0, noise_level2)
            noisy_coord_dict[key] = [noisy_y, noisy_z, values[2]]
    elif isinstance(coord_dict, np.ndarray): # If it's a numpy array, convert to dictionary first
      temp_dict = {i: [coord_dict[i][0], coord_dict[i][1], 4.0] for i in range(len(coord_dict))}
      for key, values in temp_dict.items():
          noisy_y = values[0] + np.random.normal(0, noise_level1)
          noisy_z = values[1] + np.random.normal(0, noise_level2)
          noisy_coord_dict[key] = [noisy_y, noisy_z, values[2]]
    elif isinstance(coord_dict, list):
      temp_dict = {i: [coord_dict[i][0], coord_dict[i][1], 4.0] for i in range(len(coord_dict))}
      for key, values in temp_dict.items():
          noisy_y = values[0] + np.random.normal(0, noise_level1)
          noisy_z = values[1] + np.random.normal(0, noise_level2)
...
def add_noise(filepath, headers_to_read, pca_IR, pca_OR, pca_OB, noise_level_1, noise_level_2):
    """
    Adds noise to data, performs spline interpolation, and applies PCA.
    """
    BYPASS = False
    ...
    if results_dict:
        ...
        for index, row in combined_df.iterrows():
            ...
            combined_df_inner = create_coordinate_dict(inner_radius_x_df.iloc[index], inner_radius_y_df.iloc[index])
            combined_df_outer = create_coordinate_dict(outer_radius_x_df.iloc[index], outer_radius_y_df.iloc[index])
            
            # print("combined_df_inner:", combined_df_inner)
            
            spline_points_inner, spline_points_outer = pip.angle_spline_driver(combined_df_inner, combined_df_outer, 10)
            
            # print("spline_points_inner: ", spline_points_inner)

            combined_df_inner_noise = add_noise_to_coordinate_dict(spline_points_inner, noise_level_1, noise_level_2)
            combined_df_outer_noise = add_noise_to_coordinate_dict(spline_points_outer, noise_level_1, noise_level_2)
            ...
            first_elements_inner = [row[0] for row in spline_points_inner]
            second_elements_inner = [row[1] for row in spline_points_inner]
            data_row_inner = first_elements_inner + second_elements_inner
            pc_scores_IR = pca_IR.transform(pd.DataFrame([data_row_inner]))

            first_elements_outer = [row[0] for row in spline_points_outer]
            second_elements_outer = [row[1] for row in spline_points_outer]
            data_row_outer = first_elements_outer + second_elements_outer
            pc_scores_OR = pca_OR.transform(pd.DataFrame([data_row_outer]))

            combined_list_outer = process_noisy_rows(outer_y_df.iloc[index], outer_z_df.iloc[index], noise_level_1, noise_level_2)
            outer_pc_points = sav.generate_2d_coords_for_cylinder_pca(combined_list_outer, 9)
            outer_pc_points_converted = convert_format_numpy_array(outer_pc_points)

            first_elements_ob = [row[0] for row in outer_pc_points_converted]
            second_elements_ob = [row[1] for row in outer_pc_points_converted]
            data_row_ob = first_elements_ob + second_elements_ob
            pc_scores_OB = pca_OB.transform(pd.DataFrame([data_row_ob]))
